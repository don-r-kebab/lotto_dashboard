...
def main(**argv):
    ...
    with row2:
        ...
        for col in columns:
            ...
            with col.expander(
                ...
                if view == "Today's Stats":
                    stats.todays_stats(col, client, conf)
                elif view == "Call SUT":
                    stats.sut_call_table(col, positions, ACCOUNT_DATA['Max_Short_Units'])
                elif view == "Put SUT":
                    stats.sut_data_table(col, positions, ACCOUNT_DATA['Max_Short_Units'], contract_type="PUT")
            i += 1
    ...
    with st.expander(
        ...
        view = st.selectbox(
            "What would you like ot show here?",
            (
                "Nothing",
                "Call SUT",
                "Put SUT"
    ...
    with st.expander(
        ...
        #    index=2
        #)
        min_otm_select_value = st.slider(
            label="Min Percent OTM",
            min_value = 5,
            max_value=50,
            step=5,
            value=30
